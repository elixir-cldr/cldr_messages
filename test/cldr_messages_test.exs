defmodule Cldr_Messages_Test do
  ...
  test "arguments are inserted positionally" do
    ...
    } à {3}", ["You", 3, "female", "three"], []) == {:ok, ["You", " ", "sont allées", " à ", "three"]}
  end
  ...
  test "numbers in percent format" do
    ...
    }", taxable_area: "yes", tax_rate: 0.15) == {:ok, ["An additional 15% tax will be collected."]}
  end
  ...
  test "explicit selector" do
    assert Cldr.Message.format("You have {:item_count, plural,
        ...
        one {1 item}
        other {{:item_count} items}
    }", item_count: 0) == {:ok, ["You have ", "no items"]}
  end
  ...
  test "other keyword selector" do
    assert Cldr.Message.format("You have {:item_count, plural,
        ...
        one {1 item}
        other {{:item_count} items}
    }", item_count: 2) == {:ok, ["You have ", "2 items"]}
  end

  test "nested complex arguments" do
    assert Cldr.Message.format("{gender_of_host, select, 
      female {
        {num_guests, plural, 
      ...
      %{"gender_of_host" => "male", "host" => "kip", "guest" => "jim", "num_guests" => 1}) == {:ok, ["kip invites jim to his party."]}
  end
  ...
  test "select ordinal" do
    assert Cldr.Message.format("It's my cat's {:year, selectordinal,
        ...
        few {#rd}
        other {#th}
    } birthday!", year: 3) == {:ok, ["It's my cat's ", "3rd", " birthday!"]}
  end
